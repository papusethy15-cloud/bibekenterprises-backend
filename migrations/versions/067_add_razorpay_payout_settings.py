...
Seeds the platform_settings table with Razorpay Payout (X) API config keys
...
# (group, key, value, is_secret, label)
_KEYS = [
    ('payment', 'razorpay_payout_enabled',   'false', False, 'Enable automatic payouts via Razorpay X (true/false)'),
    ...
    ('payment', 'razorpay_x_account_number', '',      False, 'Razorpay X fund account number for outgoing payouts'),
...
def upgrade():
    conn = op.get_bind()
    for group, key, value, is_secret, label in _KEYS:
        conn.execute(sa.text("""
            INSERT INTO system_settings (group, key, value, is_secret, label)
            VALUES (:group, :key, :value, :is_secret, :label)
            ON CONFLICT (group, key) DO NOTHING
        """), {"group": group, "key": key, "value": value, "is_secret": is_secret, "label": label})


def downgrade():
    conn = op.get_bind()
    for group, key, *_ in _KEYS:
        conn.execute(sa.text("DELETE FROM system_settings WHERE \"group\" = :group AND key = :key"), {"group": group, "key": key})
