...
def _normalize_mobile(mobile: str) -> list[str]:
    """Return a list of mobile number variants to try in DB lookups.
    
    The DB may store numbers as:
      - '7008691022'       (10-digit, no prefix)
      - '+917008691022'    (E.164 with + prefix)
      - '917008691022'     (with country code, no +)
    
    Flutter sends the bare 10-digit number. We search all variants so
    the backend works regardless of how the admin added the technician.
    """
    # Strip leading + or 91
    stripped = mobile.lstrip('+')
    if stripped.startswith('91') and len(stripped) == 12:
        ten_digit = stripped[2:]
    else:
        ten_digit = stripped[-10:]  # take last 10 digits
    return [
        ten_digit,           # 7008691022
        f'+91{ten_digit}',   # +917008691022
        f'91{ten_digit}',    # 917008691022
    ]
...
async def technician_send_otp(payload: SendOTPRequest, db: AsyncSession = Depends(get_db)):
    # Try all mobile number formats stored in DB (e.g. 7008691022 / +917008691022 / 917008691022)
    user = None
    for mobile_variant in _normalize_mobile(payload.mobile):
        result = await db.execute(select(User).where(User.mobile == mobile_variant))
        user = result.scalar_one_or_none()
        if user:
            break

    if not user or user.role.value != "TECHNICIAN":
        # Return 404 so the Flutter app can show a clear error message.
        # The message is generic enough to avoid disclosing registration status.
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="This mobile number is not registered as a technician. Please contact your administrator."
        )
    if not user.is_active:
        raise HTTPException(
    ...
    # Store OTP keyed to the canonical mobile (what the app sends) so verify-otp can find it
    otp = await _issue_otp(payload.mobile)
    # TODO: send via SMS gateway. For now, otp is returned in body so the app
    # can show the yellow dev-mode banner and auto-fill the OTP.
    return success_response(
        data={"otp": otp, "expires_in": f"{settings.OTP_EXPIRE_MINUTES} minutes"},
        message="OTP sent successfully"
    )
...
async def technician_verify_otp(payload: VerifyOTPRequest, db: AsyncSession = Depends(get_db)):
    ...
    if not stored_otp or stored_otp != payload.otp:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid or expired OTP")
    await r.delete(key)
    # Search all mobile format variants (same logic as send-otp) so verify
    # succeeds regardless of how the number was stored in the DB.
    user = None
    for mobile_variant in _normalize_mobile(payload.mobile):
        result = await db.execute(select(User).where(User.mobile == mobile_variant))
        user = result.scalar_one_or_none()
