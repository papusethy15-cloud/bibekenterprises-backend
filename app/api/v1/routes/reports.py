...
async def technician_detail_report(
    ...
    wallet_txn_rows = [{
        ...
        "balance_before":  round(t.balance_before or 0, 2),
        "balance_after":   round(t.balance_after or 0, 2),
    ...
    return success_response(data={
        ...
        "ratings":      rating_rows,
        "wallet": {
            "balance":         round(wallet_row.balance or 0, 2) if wallet_row else 0,
            "total_earned":    round(wallet_row.total_earned or 0, 2) if wallet_row else 0,
            "total_withdrawn": round(wallet_row.total_withdrawn or 0, 2) if wallet_row else 0,
        } if wallet_row else None,
        "wallet_summary": {
            "credits_in_period":      round(wallet_credits, 2),
            "debits_in_period":       round(wallet_debits, 2),
            "withdrawals_in_period":  round(wallet_withdrawals, 2),
